...
class star():
    ...
    def update(self):
        ...
        if time() - self.start > self.timeout:
            ...
            new_cells = [(i,h+j) for j in range(-self.size,self.size) for i in (w-abs(j), w+abs(j)) ]
...
class cell():
    ...
    def draw(self):
        ...
        if b1 > 0 or b2 > 0:
            pygame.draw.rect(self.conf['screen'], self.conf['line_color'], (x, y, w+b1+b2, w+b1+b2)) 
        if self.active:
            pygame.gfxdraw.box(self.conf['screen'], (x+b1, y+b1, w, w), self.color)
        else:
            pygame.draw.rect(self.conf['screen'], self.conf['bg_color'], (x+b1, y+b1, w, w))
        if self.display_text:
            txt = self.conf['cfont'].render(f'{self.pos}', True, self.conf['text_color'])
            a, b = txt.get_size()
...
def init(conf):
    ...
    # Create the window
    conf['screen'] = pygame.display.set_mode(conf['size'], pygame.DOUBLEBUF | pygame.RESIZABLE)
...
def draw(conf, table, star_list):
    ...
    if x < w or y < w:
        ...
        for i in range(b1+b2):
            pygame.draw.rect(conf['screen'], conf['line_color'], (x-b2+i, y-b2+i, ex+b1+b2-2*i, ey+b1+b2-2*i), width=1)
    for c in table:
        c.draw()
    for c in star_list:
        c.draw()
    ...
    w, h = conf['size']
    conf['screen'].blit(conf['font'].render(f'Time:', True, conf['text_color']), (w-100, 5))
...
def main(conf):
    ...
    while True:
        ...
        for s in stars:
            if not s.live:
                stars.remove(s)
        sys.stdout.write(f'\r load index: {((time()-t1)/tpf)*100:0.2f}% \t\t star list length: {len(stars)}')
